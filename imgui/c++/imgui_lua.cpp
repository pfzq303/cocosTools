...
static int imgui_plotLines(lua_State *L) {
	...
	if (args > 2) {
		values_offset = luaL_checknumber(L, 3);
	}
	if (args > 3) {
		overlay_text = luaL_checkstring(L, 4);
	}
	if (args > 4) {
		scale_min = luaL_checknumber(L, 5);
	}
	if (args > 5) {
		scale_max = luaL_checknumber(L, 6);
	}
	if (args > 7) {
		graph_size = ImVec2(luaL_checknumber(L, 7), luaL_checknumber(L, 8));
	}
	ImGui::PlotLines(label, arr.data(), arr.size() , values_offset , overlay_text , scale_min , scale_max , graph_size);
	return 0;
}
...
static int imgui_plotLine(lua_State *L) {
...
static int imgui_selectable(lua_State *L) {
	lua_pushboolean(L, ImGui::Selectable(luaL_checkstring(L, 1)));
	return 1;
}
static int imgui_pushItemWidth(lua_State *L) {
	ImGui::PushItemWidth(luaL_checknumber(L, 1));
	return 0;
}
...
static int imgui_showDemo(lua_State *L) {
...
static int imgui_alignFirstTextHeightToWidgets(lua_State *L) {
...
static int imgui_colorEdit(lua_State *L) {
	const char *label = luaL_checkstring(L, 1); 
	int args = lua_gettop(L);
	if (args > 4) {
		float f[4] = { (float)luaL_checknumber(L, 2), (float)luaL_checknumber(L, 3), (float)luaL_checknumber(L, 4) , (float)luaL_checknumber(L, 5) };
		lua_pushboolean(L, ImGui::ColorEdit4(label, f));
		lua_pushnumber(L, f[0]);
		lua_pushnumber(L, f[1]);
	...
	{
		float f[3] = { (float)luaL_checknumber(L, 2), (float)luaL_checknumber(L, 3), (float)luaL_checknumber(L, 4) };
		lua_pushboolean(L, ImGui::ColorEdit3(label, f));
		lua_pushnumber(L, f[0]);
		lua_pushnumber(L, f[1]);
		lua_pushnumber(L, f[2]);
		return 4;
...
static int imgui_beginPopupContextItem(lua_State *L) {
	...
	int button = 1;
	if (args > 1) {
		button = luaL_checknumber(L, 2);
	}
	lua_pushboolean(L, ImGui::BeginPopupContextItem(luaL_checkstring(L, 1), button));
	return 1;
}
...
#define FIELD(field) \
	...
	FIELD(ImGuiStyleVar_Alpha)
	FIELD(ImGuiStyleVar_WindowPadding)
	FIELD(ImGuiStyleVar_WindowRounding)
	FIELD(ImGuiStyleVar_WindowMinSize)
	FIELD(ImGuiStyleVar_ChildWindowRounding)
	FIELD(ImGuiStyleVar_FramePadding)
	FIELD(ImGuiStyleVar_FrameRounding)
	FIELD(ImGuiStyleVar_ItemSpacing)
	FIELD(ImGuiStyleVar_ItemInnerSpacing)
	FIELD(ImGuiStyleVar_IndentSpacing)
	FIELD(ImGuiStyleVar_GrabMinSize)
